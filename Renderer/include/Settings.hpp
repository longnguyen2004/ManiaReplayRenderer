...
public:
    ...
    explicit Settings(const std::string &pathToOsuFile, std::string section);
 ...
 * and strings, but parsing the settings alone doesn't allow you to know which
 * setting is which. For the sake of simplicity and to avoid hardcoding types,
 * I end up making the value type std::string, and rely on the user to convert
